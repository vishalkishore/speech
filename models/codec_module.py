...
from .norms import CausalFreqNorm  
...
class DenseBlock(nn.Module):
    ...
    def __init__(self, cfg, kernel_size=(3, 3), depth=4):
        ...
        self.cfg = cfg
        self.depth = depth
        self.causal = cfg['model_cfg'].get('causal', False)
        ...
        for i in range(depth):
            dil = 2 ** i
            freq_pad = get_padding_2d(kernel_size, (dil, 1))[1]
            dense_conv = nn.Sequential(
                nn.Conv2d(
                    ...
                    kernel_size,
                    dilation=(dil, 1),
                    padding=(0, freq_pad) if self.causal else get_padding_2d(kernel_size, (dil, 1)),
                ),
                CausalFreqNorm(self.hid_feature, affine=True),
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            if self.causal:
                dil = 2 ** i
                time_pad = (self.kernel_size[0] - 1) * dil
                skip_in = F.pad(skip, (0, 0, time_pad, 0))
            else:
                skip_in = skip
            x = self.dense_block[i](skip_in)
...
class MagDecoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mask_conv = nn.Sequential(
            ...
            CausalFreqNorm(self.output_channel, affine=True),
