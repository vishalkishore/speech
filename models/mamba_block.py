...
class MambaBlock(nn.Module):
    def __init__(self, in_channels, cfg, bidirectional=True):
        ...
        self.backward_blocks = nn.ModuleList( create_block(in_channels, cfg) for i in range(n_layer) ) if bidirectional else None
...
class TFMambaBlock(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        time_channels = self.hid_feature if self.causal else self.hid_feature * 2
        self.tlinear = nn.ConvTranspose1d(time_channels, self.hid_feature, 1, stride=1)
